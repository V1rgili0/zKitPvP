...
public class DropSettingsCommand extends KitPvPCommand {
    ...
    public boolean execute(CommandSender sender, List<String> args) {
        ...
        }).addItemStack(14, getArrow(data)).addAction(14, event -> {
            Inventory clickedInventory = event.getClickedInventory();
            PlayerData clickerData = KitPvP.getDataManager().getPlayerData(event.getWhoClicked().getUniqueId());
    ...
    private ItemStack getArrow(PlayerData data) {
        ...
        this.lines.forEach(lore -> arrow.addLoreLine(ChatUtils.getColoredText(lore.replaceAll("%enabled%", KitPvP.getFileManager().getConfig().getString("drop-settings." + (data.pickupArrows ? "enabled" : "disabled") + "-text")))));
        return arrow.get();
    }

    private ItemStack getGoldenApple(PlayerData data) {
        ItemMaker apple = new ItemMaker(Material.GOLDEN_APPLE);
        this.lines.forEach(lore -> apple.addLoreLine(ChatUtils.getColoredText(lore.replaceAll("%enabled%", KitPvP.getFileManager().getConfig().getString("drop-settings." + (data.pickupGoldenApple ? "enabled" : "disabled") + "-text")))));
        return apple.get();
    }
}
